...
export function CoachContentScreen({ user, onBack }) {
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div>
        <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.14em" }}>Forge Academy</div>
        <div style={{ fontFamily: BRAND.display, fontSize: 26, fontWeight: 500, letterSpacing: "-0.01em", color: BRAND.text }}>{editId ? "Edit article" : "Write an article"}</div>
      </div>
      <Card style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {cats.map((c) => {
            ...
            return <button key={c} onClick={() => setCat(c)} style={{ fontFamily: BRAND.sans, fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.1em", color: selected ? "var(--btn-ink)" : BRAND.muted, background: selected ? "var(--btn-bg)" : BRAND.card2, border: `${BRAND.hairline} solid ${selected ? "transparent" : BRAND.line}`, borderRadius: 999, padding: "8px 12px", cursor: "pointer" }}>{c}</button>;
          })}
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Button onClick={publish} disabled={saving} style={{ flex: 1 }}>{saving ? "Publishing..." : editId ? "Update article" : "Publish to all clients"}</Button>
          {editId && <Button variant="dark" onClick={() => { setEditId(null); setTitle(""); setBody(""); }}>Cancel</Button>}
        </div>
      </Card>
      <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.14em" }}>Published ({articles.length})</div>
      ...
      {articles.map((a) => (
        <Card key={a.id} flat style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
            <Button variant="dark" onClick={() => edit(a)}>Edit</Button>
            <Button variant="red" onClick={() => remove(a.id)}>x</Button>
          </div>
        </Card>
      ))}
    </div>
...
export function LearnTab({ client }) {
  ...
  const header = (
    <div>
      <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.14em" }}>Forge Academy</div>
      <div style={{ fontFamily: BRAND.display, fontSize: 26, fontWeight: 500, letterSpacing: "-0.01em", color: BRAND.text }}>Learn</div>
    </div>
  );
  if (open) {
    return (
      <div style={{ display: "grid", gap: 12 }}>
        <Card>
          <button onClick={() => setOpen(null)} style={{ fontFamily: BRAND.sans, background: "transparent", border: "none", color: BRAND.blue, fontWeight: 500, fontSize: 12, cursor: "pointer", padding: 0 }}>{"< Back"}</button>
          <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.14em", marginTop: 14 }}>{open.cat} · {open.read} read</div>
          <div style={{ fontFamily: BRAND.display, fontSize: 24, fontWeight: 500, letterSpacing: "-0.01em", marginTop: 8, lineHeight: 1.25, color: BRAND.text }}>{open.title}</div>
          <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, marginTop: 6 }}>The Forge Method · {open.date}</div>
          <div style={{ fontFamily: BRAND.sans, color: BRAND.text, fontSize: 14, lineHeight: 1.6, marginTop: 16, whiteSpace: "pre-line", fontWeight: 400 }}>{open.body}</div>
        </Card>
      </div>
    );
  }
  if (articles === null) return <div style={{ display: "grid", gap: 12 }}>{header}<Card><div style={{ fontFamily: BRAND.sans, color: BRAND.muted, fontSize: 14 }}>Loading...</div></Card></div>;
    ...
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <>
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 2 }}>
          {cats.map((c) => {
            const selected = cat === c;
            return <button key={c} onClick={() => setCat(c)} style={{ fontFamily: BRAND.sans, whiteSpace: "nowrap", fontSize: 11, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.1em", color: selected ? "var(--btn-ink)" : BRAND.muted, background: selected ? "var(--btn-bg)" : BRAND.card2, border: `${BRAND.hairline} solid ${selected ? "transparent" : BRAND.line}`, borderRadius: 999, padding: "8px 13px", cursor: "pointer" }}>{c}</button>;
          })}
        </div>
        {list.map((a) => (
          <Card key={a.id} flat onClick={() => setOpen(a)} style={{ cursor: "pointer" }}>
            ...
            <div style={{ fontFamily: BRAND.sans, fontWeight: 500, fontSize: 16, marginTop: 8, lineHeight: 1.25, color: BRAND.text }}>{a.title}</div>
            <div style={{ fontFamily: BRAND.sans, color: BRAND.dim, fontSize: 11, fontWeight: 500, marginTop: 8 }}>{a.read} read · {a.date}</div>
          </Card>
        ))}
        {list.length === 0 && <Card><div style={{ fontFamily: BRAND.sans, color: BRAND.muted, fontSize: 14 }}>Nothing in {cat} yet.</div></Card>}
      </>
    </div>
  );
}
