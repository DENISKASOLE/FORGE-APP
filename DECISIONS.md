# Teal/deep-green brand pivot — decisions log

Ask: "from now on the app is redesigned from scratch screen by screen with
brand new colors (dark teal/deep green,) is the brand color of the app.
match it with other colors that complements it." Autonomous, no
clarifying questions. Reference: a screenshot of an unrelated multi-branch
gym-analytics dashboard ("RESTART") with a dark teal sidebar.

## Scope: colors, not layout

The reference screenshot is a completely different kind of app — a
desktop sidebar-nav analytics dashboard for managing multiple gym
branches — structurally nothing like Forge (a mobile-first, bottom-nav
single-trainer coaching app). Treated it as a color/mood reference only
(the dark teal-green tone, paired with clean semantic status colors), not
a layout to rebuild Forge's screens around. "Redesigned from scratch
screen by screen" is interpreted as "every screen picks up the new brand
color system," not "every screen's structure is rebuilt" — nothing in the
request described new layouts, information architecture, or content for
any specific screen, only a new color identity.

## One token change, not a screen-by-screen rewrite

Confirmed again (third time this project) that every screen is already
100% color-token-driven through `BRAND`/`T` in `src/theme/tokens.js` ->
CSS custom properties in `src/styles/theme.css` — a repo-wide grep for
`BRAND.gold` background paired with a hardcoded (non-token) text color
found zero hits, confirming the "always pair a `BRAND.gold` fill with
`BRAND.btnInk`, never a literal `#fff`/`#000`" rule from the glass-restyle
DECISIONS.md entry has held. So this was genuinely a `theme.css`-only
change - editing `--accent` (and everything that reads from it) recolors
every button, active nav/tab state, focus ring, and text-emphasis spot in
the app at once.

## Palette

- **Brand accent** (`--accent`): `#16A88A` dark theme / `#0B6B58` light
  theme — a rich teal-emerald, not a pure/neon green. Deliberately kept
  distinct in hue from the existing semantic "success" green
  (`#5FBE86`/`#2E9B68`, used for training completion, checkmarks, "good"
  states) so the brand color and the "something succeeded" signal don't
  collide — a mid-tone teal (blue-leaning) reads as clearly different
  from a yellow-leaning green even sitting near each other.
- Light/dark got **inverted contrast tuning**, matching how this token
  already worked before the pivot (dark theme's accent was white, light
  theme's was near-black, each picked for contrast against its own page
  color): dark theme gets the *brighter* teal so it pops on a near-black
  page; light theme gets a *deeper* teal so it reads on a near-white page.
  `--btn-ink` also had to change from a fixed black/white pair to
  something that actually contrasts against a mid-saturation color rather
  than a pure black/white fill — dark theme buttons now use a near-black
  teal ink (`#04140F`) rather than pure black, light theme keeps white.
- **Structural surfaces weren't left neutral.** Rather than only recolor
  buttons and leave the rest of the app monochrome gray/white (which
  would read as "a teal button was bolted onto the old design" rather
  than "the app is teal-branded"), gave the page background, glass-card
  frost, hairlines, and the cinematic ambient backdrop a whisper of the
  brand hue each (e.g. dark page shifted from pure `#000000` to
  `#040D0B`, glass borders/frost shifted from white-alpha to a pale
  teal-alpha, the cinematic radial glow switched from white to
  `rgba(22,168,138,...)`). Kept these very subtle - the goal is "the
  whole app has a teal identity," not "everything is visibly green."
- **Left the functional/semantic accent set alone**
  (green/blue/yellow/violet/red/orange - success, action links, warning/
  fuel, recovery/dinner, danger, breakfast) and the Home-screen momentum-
  ring palette (`HOME_ACCENT`/`HOME_TRACK` in `ClientView.jsx`) untouched.
  Those are deliberate, already-validated meaning-signals from prior work,
  not brand-identity color — repainting them teal would both break their
  distinct-from-brand contrast (see above) and go against the "accents
  are signals, not decoration" rule from the original glass restyle.
- `.glass`/`.glass-soft`/`.glass-nav`'s frost/border tint got the same
  subtle teal treatment; the `forge-glow-pulse` hero-card glow (previously
  a hardcoded white pulse) now pulses in `var(--accent)` via
  `color-mix()`, so it's automatically teal in dark mode and would follow
  any future accent change without another manual edit.

## Verification

`npm run build` clean. Grepped for hardcoded `rgba(255,255,255,...)` usage
app-wide first to make sure nothing else assumed "the accent is always
literally white" - the only two hits (`Button.jsx`'s glossy button-sheen
highlight, and a video-thumbnail play-button overlay in
`SupersetLogger.jsx`) are both meant to stay a literal white regardless of
theme (a light reflection and a video-player icon respectively, not brand
statements), so left alone. Screenshotted both themes via a static
`vite preview` build (same approach as the two prior recolors, since the
dev server's HMR reconnects have proven unreliable for scripted
screenshots in this sandbox) - dark mode shows a near-black teal page with
a visible ambient glow, teal "Sign in" button, teal input borders, and
teal link text; light mode mirrors it with a deep-teal button on a
mint-white page. Zero console/page errors in both.
